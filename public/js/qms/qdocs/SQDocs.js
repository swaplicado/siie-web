var docsApp = new Vue({
  ...
  methods: {
    ...
    viewFile(idConf, idField) {
      var _img = document.getElementById(
        this.lResults[idConf + "_" + idField].id_tag
      );
      if (
        this.lResults[idConf + "_" + idField].data == null ||
        this.lResults[idConf + "_" + idField].data.length == 0
      ) {
    ...
    guardar(idConf, idField) {
      ...
      let imagefile = document.getElementById(
        this.lResults[idConf + "_" + idField].id_tag + "1"
      );
      ...
      axios
        ...
        .then(res => {
          if (res.data == "Error") {
            oGui.showError(
              "Verifique que sea un formato de imagen válido y que el tamaño del archivo no sea mayor a 2048 KB"
            );
          } else {
  ...
  mounted: function() {
    let results = [];
    if (
      this.vMongoDocument == null ||
      (this.vMongoDocument != null &&
        oData.scqms.CFG_ZONE.FQ == oData.cfgZone &&
        this.vMongoDocument.results == undefined) ||
      (this.vMongoDocument != null &&
        oData.scqms.CFG_ZONE.MB == oData.cfgZone &&
        this.vMongoDocument.resultsMb == undefined)
    ) {
      for (const config of this.vlConfigurations) {
        for (const field of config.lFields) {
          let oResult = new SResult(
            config.id_configuration,
            field.id_field,
            null
          );
          ...
          results[
            "" + config.id_configuration + "_" + field.id_field
          ] = oResult;
        }
      }
