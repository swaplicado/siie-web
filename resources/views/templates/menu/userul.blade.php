<ul class="nav navbar-nav navbar-right">
  <li class="dropdown">
    ...
    <ul class="dropdown-menu">
      ...
      <li><a href="{{ route('start') }}">{{"Sucursal :"}}<?php echo session()->has('branch') ? session('branch')->name : '' ?></a></li>
      <li><a href="{{ route('start') }}">{{"Almacen :"}}<?php echo session()->has('whs') ? session('whs')->name : '' ?></a></li>
